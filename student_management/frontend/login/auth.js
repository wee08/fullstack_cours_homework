...
(() => {
  ...
  function isValidEmail(value){ return EMAIL_RE.test(value.trim()); }
  ...
  function setFieldState(fieldEl, state){ // state: 'valid' | 'invalid' | 'neutral'
  ...
  function wireLiveValidation(fieldEl, validatorFn, opts){
    const input = fieldEl.querySelector('input');
    input.addEventListener('blur', () => validateField(fieldEl, validatorFn, opts));
    input.addEventListener('input', () => {
      if (fieldEl.classList.contains('is-invalid') && input.value.trim() && validatorFn(input.value)){
        setFieldState(fieldEl, 'valid');
      } else if (fieldEl.classList.contains('is-invalid') === false && fieldEl.classList.contains('is-valid') === false){
  ...
  signupPassword && signupPassword.addEventListener('input', () => {
    ...
    const colors = ['var(--color-danger)', 'var(--color-danger)', 'var(--color-warning)', 'var(--color-warning)', 'var(--color-success)', 'var(--color-success)'];
    if (window.gsap){
      gsap.to(pwFill, { width: pct + '%', backgroundColor: colors[score], duration: 0.3, ease: easeOut });
    } else {
      pwFill.style.width = pct + '%';
      pwFill.style.background = colors[score];
    }
    pwLabel.textContent = signupPassword.value ? labels[score] : 'Password strength';
  });
  ...
  document.querySelectorAll('.role-switch').forEach(group => {
    ...
    requestAnimationFrame(() => movePill(group.querySelector('.role-option.is-active'), false));
    window.addEventListener('resize', () => movePill(group.querySelector('.role-option.is-active'), false));
  });
  ...
  function switchTab(target){
    ...
    if (window.gsap){
      gsap.to(tabIndicator, { x: toLogin ? 0 : '100%', duration: 0.3, ease: easeOut });
    } else {
    ...
    if (window.gsap){
      gsap.to(hideEl, { opacity: 0, y: -8, duration: 0.16, ease: 'power2.in', onComplete: () => {
        hideEl.classList.remove('is-active');
        showEl.classList.add('is-active');
        gsap.fromTo(showEl, { opacity: 0, y: 8 }, { opacity: 1, y: 0, duration: 0.28, ease: easeOut });
      }});
    } else {
      hideEl.classList.remove('is-active');
      showEl.classList.add('is-active');
  ...
  const loginPasswordField = document.querySelector('[data-field="loginPassword"]');
  ...
  const signupPasswordField= document.querySelector('[data-field="signupPassword"]');
  const signupConfirmField = document.querySelector('[data-field="signupConfirm"]');
  ...
  wireLiveValidation(signupConfirmField, v => v === document.getElementById('signupPassword').value && v.length >= 8);
  ...
  document.getElementById('signupPassword').addEventListener('input', () => {
    ...
    if (confirmInput.value){
      const ok = confirmInput.value === document.getElementById('signupPassword').value;
  ...
  function showToast(message, icon = 'check-circle-2'){
    ...
    toastTimer = setTimeout(() => {
      if (window.gsap){
        gsap.to(toastEl, { y: 24, opacity: 0, duration: 0.25, ease: 'power2.in' });
      } else {
  ...
  signupForm.addEventListener('submit', (e) => {
    ...
    const confirmOk = validateField(signupConfirmField, v => v === document.getElementById('signupPassword').value && v.length >= 8);

    const termsOk = termsCheckbox.checked;
    termsError.classList.toggle('is-visible', !termsOk);
    if (!termsOk && window.gsap) shake(termsCheckbox.closest('.checkbox-field--terms'));
  ...
  wireHoverScale('.role-option', 1.0); // pill provides the visual feedback

  /* ---------------- Entrance animation ---------------- */
  function pageLoadAnimation(){
    if (!window.gsap) return;
    const tl = gsap.timeline({ defaults: { ease: easeOut } });
    tl.fromTo(authCard, { opacity: 0, y: 24, scale: 0.98 }, { opacity: 1, y: 0, scale: 1, duration: 0.5 })
      .fromTo('.auth-visual-top, .auth-mobile-brand', { opacity: 0, y: -8 }, { opacity: 1, y: 0, duration: 0.35 }, '-=0.35')
      .fromTo('.auth-visual-body h2, .auth-visual-body p', { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.4, stagger: 0.08 }, '-=0.2')
      .fromTo('.auth-visual-card', { opacity: 0, y: 16 }, { opacity: 1, y: 0, duration: 0.4 }, '-=0.15')
      .fromTo('.auth-panel.is-active .field, .auth-panel.is-active .role-switch, .auth-panel.is-active .auth-row, .auth-panel.is-active .checkbox-field--terms',
        { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.32, stagger: 0.05 }, '-=0.25');
  }
